...
+ [`resumable`](https://github.com/SeaQL/sea-streamer/blob/main/examples/src/bin/resumable.rs): 一个可恢复的流处理器，从中断处继续
+ [`buffered`](https://github.com/SeaQL/sea-streamer/blob/main/examples/src/bin/buffered.rs): 一个具有内部缓冲和批处理的高级流处理器
+ [`blocking`](https://github.com/SeaQL/sea-streamer/blob/main/examples/src/bin/blocking.rs): 一个用于处理阻塞/CPU密集型任务的高级流处理器
...
例如，向数据库插入记录，批量插入更高效。但是你不能天真地将批量大小固定为10或100，因为它可能已经缓存了9条消息，正在等待第10条，而你无法处理突然的消息爆发。
