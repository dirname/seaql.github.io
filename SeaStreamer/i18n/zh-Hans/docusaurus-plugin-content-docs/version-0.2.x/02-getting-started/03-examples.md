...
可恢复的处理器可以随时被杀死，并将从中断处继续。这通常被称为“至少一次”处理，意味着不应跳过任何消息，但同一条消息可能被处理两次。
...
时钟运行速度比处理器快10倍，因此我们期望每个批次大约包含10条消息。

当输入流频率很高，而处理器阻抗很大时，这种模式非常有用。

<details>
  <summary>更深入的技术讨论：</summary>
例如，向数据库中插入记录时，批量插入更为高效。但是你不能简单地将批处理大小固定为10或100，因为它可能已经缓存了9条消息，正在等待第10条，而且你无法处理突然的消息爆发。

那么，如何最小化总体任务执行时间呢？你需要解耦两个繁忙循环，并使用队列来松散地连接它们：现在两个循环都可以以它们的最佳频率旋转，从而最大化处理器的整体吞吐量。
</details>
...
时钟运行速度比处理器快3倍，但我们有4个线程，因此我们预计它能够实时跟上。任务随机分配给线程，即所谓的“扇出”模式。

当你需要执行阻塞IO或CPU密集型计算时，这种模式非常有用。
