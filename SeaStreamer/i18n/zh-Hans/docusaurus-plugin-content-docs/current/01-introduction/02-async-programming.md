...
第二个需要学习的概念是 [`Stream`](https://docs.rs/futures-core/latest/futures_core/stream/trait.Stream.html) 特性。它类似于 [`Iterator`](https://doc.rust-lang.org/std/iter/trait.Iterator.html)，但它是异步的。它非常强大，允许我们通过组合 `Map`、`Filter` 和 `Fold` 来以人体工程学的方式操纵流。
