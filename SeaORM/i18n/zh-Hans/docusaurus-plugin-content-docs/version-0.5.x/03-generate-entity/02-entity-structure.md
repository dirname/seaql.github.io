...
| Rust 类型 | 数据库类型 <br/> ([`ColumnType`](https://docs.rs/sea-orm/0.5/sea_orm/entity/enum.ColumnType.html)) |
| --------- | ------------- |
| String | Char |
| String | String |
| u8, i8 | TinyInteger |
...
你可以使用 `column_type` 属性来覆盖 Rust 类型和 `ColumnType` 之间的默认映射。
...
你可以向列添加额外的属性 `unique`、`indexed` 和 `nullable`。

如果你为可选属性指定了自定义的 `column_type`，你也必须指定 `nullable`。
...
默认情况下，对于 `primary_key` 列，`auto_increment` 是隐含的。通过指定 `false` 来覆盖它。
...
这通常发生在连接表中，其中使用两列元组作为主键。只需注解多个列来定义复合主键。默认情况下，复合键的 `auto_increment` 是 `false`。
