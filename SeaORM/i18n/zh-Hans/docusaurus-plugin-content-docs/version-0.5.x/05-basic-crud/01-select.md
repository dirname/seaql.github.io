...
通过其主键查找模型，它可以是单个键或复合键。我们从调用 `Entity` 上的 [`find_by_id`](https://docs.rs/sea-orm/0.5/sea_orm/entity/trait.EntityTrait.html#method.find_by_id) 开始，这将帮助你自动构建查询和条件。然后，使用 `one` 方法从数据库中获取单一模型。
...
> 阅读更多关于 [表连接](08-advanced-query/04-custom-joins.md) 的内容。
...
所有相关模型一次性加载。这与懒加载相比，在数据库往返次数上提供了最小的开销。
...
将任何 SeaORM 查询转换为具有自定义页面大小的 [分页器](https://docs.rs/sea-orm/0.5/sea_orm/struct.Paginator.html)。
