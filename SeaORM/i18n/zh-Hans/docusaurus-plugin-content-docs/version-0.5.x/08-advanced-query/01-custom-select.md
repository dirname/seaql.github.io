...
根据需要通过调用 `select_only` 方法来清除默认选取。然后，你可以在其后选取一些属性或甚至自定义表达式。
...
使用 `column_as` 方法选取任何自定义表达式，它接受任何 [`sea_query::SimpleExpr`](https://docs.rs/sea-query/*/sea_query/expr/enum.SimpleExpr.html) 和一个别名。使用 [`sea_query::Expr`](https://docs.rs/sea-query/*/sea_query/expr/struct.Expr.html) 辅助函数来构建 `SimpleExpr`。
