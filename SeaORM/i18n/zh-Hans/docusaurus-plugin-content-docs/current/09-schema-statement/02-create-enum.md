...
<details>
    <summary>请注意，非 UAX#31 符合的字符将被转换，如下面所示。</summary>
...
PostgreSQL 中的枚举由 [`TypeCreateStatement`](https://docs.rs/sea-query/*/sea_query/extension/postgres/struct.TypeCreateStatement.html) 定义，可以从 `Entity` 通过 `Schema::create_enum_from_entity` 方法创建。

您也可以从 `ActiveEnum` 通过 `Schema::create_enum_from_active_enum` 方法创建它。
...
在 MySQL 中，枚举是在创建表时定义的，因此只需调用一次 `Schema::create_table_from_entity`。
