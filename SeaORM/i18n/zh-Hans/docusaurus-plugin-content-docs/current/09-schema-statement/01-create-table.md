...
为了在数据库中创建表而无需手动编写[`TableCreateStatement`](https://docs.rs/sea-query/*/sea_query/table/struct.TableCreateStatement.html)，您可以使用`Entity`的`Schema::create_table_from_entity`方法来派生它。此方法将帮助您创建数据库表，包括在`Entity`中定义的所有列和外键约束。

下面，我们使用[`CakeFillingPrice`](https://github.com/SeaQL/sea-orm/blob/master/src/tests_cfg/cake_filling_price.rs)实体演示其生成的 SQL 语句。您可以使用[`TableCreateStatement`](https://docs.rs/sea-query/*/sea_query/table/struct.TableCreateStatement.html)构建相同的语句。

请注意，自版本`0.7.0`起，`Schema::create_table_from_entity`不再创建索引。如果您需要在数据库中创建索引，请查看[这里](09-schema-statement/03-create-index.md)了解详细信息。
