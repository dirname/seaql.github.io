...
如果您想要添加一个额外的`OR`条件，可以使用`condition_type = "any"`来更改关系。
...
pub enum Relation {
    ...
    #[sea_orm(
        ...
        // 额外的on_condition，接受任何实现了`sea_query::IntoCondition`的值
    ...
    fn link(&self) -> Vec<RelationDef> {
        vec![
            super::cake_filling::Relation::Cake
                .def()
                // `on_condition`方法接受一个闭包，参数表示连接表达式中的左表和右表。
