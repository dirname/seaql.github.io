...
MSSQL 驱动程序的安装和配置详情可在此处找到：[https://www.sea-ql.org/SeaORM-X/docs/install-and-config/database-and-async-runtime/](https://www.sea-ql.org/SeaORM-X/docs/install-and-config/database-and-async-runtime/)。
...
+ `ASYNC_RUNTIME` 可以是 [`async-std`](https://crates.io/crates/async-std) 或者是 [`tokio`](https://crates.io/crates/tokio)
+ `TLS_LIB` 可以是 [`native-tls`](https://crates.io/crates/native-tls) 或者是 [`rustls`](https://crates.io/crates/rustls)

1. 根据您的 Rust Web 框架选择对应的 ASYNC_RUNTIME：
...
+ `postgres-array` - 支持 Postgres 中的数组类型（当启用 `sqlx-postgres` 功能时会自动启用）
+ `sea-orm-internal` - 允许使用不稳定的内部 API（为了访问重导出的 SQLx 类型）