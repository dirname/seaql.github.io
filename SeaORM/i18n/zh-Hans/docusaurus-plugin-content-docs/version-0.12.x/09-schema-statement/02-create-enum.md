...
<details>
    <summary>注意非 UAX #31 符合规范的字符将如下面所示进行转换。</summary>
...
PostgreSQL 中的枚举由 `TypeCreateStatement` 定义，可以从 `Entity` 使用 `Schema::create_enum_from_entity` 方法创建。

你也可以从 `ActiveEnum` 使用 `Schema::create_enum_from_active_enum` 方法创建它。
...
在 MySQL 中，枚举是在创建表时定义的，因此你只需要调用一次 `Schema::create_table_from_entity`。
