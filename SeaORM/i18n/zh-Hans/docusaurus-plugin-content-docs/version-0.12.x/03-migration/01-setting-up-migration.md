...
如果您从一个全新的数据库开始，最好对数据库模式进行版本控制。SeaORM 随附了一个迁移工具，允许您使用 SeaQuery 或 SQL 编写迁移。
...
请注意，如果直接在 Git 存储库中设置迁移目录，还将创建一个 `.gitignore` 文件。

## 工作区结构

建议按照以下方式构建您的 cargo 工作区，以便在应用程序包和迁移包之间共享 SeaORM 实体。查看 [集成示例](https://github.com/SeaQL/sea-orm/tree/master/examples) 以进行演示。
...
version = "0.12"
features = [
  # 至少启用一个 `ASYNC_RUNTIME` 和 `DATABASE_DRIVER` 功能，如果您想通过 CLI 运行迁移。
  # 查看支持的功能列表 https://www.sea-ql.org/SeaORM/docs/install-and-config/database-and-async-runtime。
  # 例如。
...
您可以创建一个不包含任何实体文件的实体包。然后，编写迁移并运行它以在数据库中创建表。最后，使用 `sea-orm-cli` [生成 SeaORM 实体](04-generate-entity/01-sea-orm-cli.md)并将实体文件输出到 `entity/src/entities` 文件夹。
...
创建一个包含应用程序、实体和迁移包的工作区，并将实体包导入到应用程序包中。

如果我们想将迁移工具作为您应用程序的一部分捆绑在一起，您还可能想要导入迁移包。
