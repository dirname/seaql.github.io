...
如果您想将用户输入保存到数据库中，可以轻松地将JSON值转换为`ActiveModel`。请注意，您可能想要[跳过反序列化](https://serde.rs/attr-skip-serializing.html)JSON的主要键属性，您可以像下面所示进行配置。
...
插入一个活动模型并返回最后插入的ID。它的类型与模型的主键类型匹配，因此如果模型具有复合主键，则可能是一个元组。
...
向`insert_many`方法提供空集将导致错误。但是，您可以使用`on_empty_do_nothing`来改变行为，它将`InsertResult`包装在`TryInsertResult`中。
...
执行不插入或更新任何行的upsert语句将导致`DbErr::RecordNotInserted`错误。
...
如果您希望`RecordNotInserted`是`Ok`而不是错误，请调用`.do_nothing()`：
