...
为了获取数据库连接，请使用[`Database`](https://docs.rs/sea-orm/*/sea_orm/struct.Database.html)接口：
...
如果你无法使 `localhost` 工作，尝试输入 IP 地址和端口号，例如 `127.0.0.1:3306` 或者甚至是 `192.168.x.x`。
...
每次你调用 `execute` 或 `query_one/all`，都会从池中获取和释放一个连接。
...
opt.max_connections(100)
    ...
    .set_schema_search_path("my_schema"); // 设置默认的 PostgreSQL 模式
...
连接会在被丢弃时自动关闭。要显式地关闭连接，请调用 `close` 方法。
