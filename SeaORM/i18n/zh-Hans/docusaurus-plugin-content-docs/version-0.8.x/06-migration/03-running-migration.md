...
迁移可以在终端中手动运行。必须在您的环境中设置 `DATABASE_URL`，请按照[此处](03-generate-entity/01-sea-orm-cli.md#configure-environment)的说明进行配置。
...
您可以使用实现了[`MigratorTrait`](https://docs.rs/sea-orm-migration/0.8/sea_orm_migration/migrator/trait.MigratorTrait.html)的 `Migrator` 在应用程序启动时执行迁移。
