...
使用 `column_as` 方法选取任何自定义表达式，它接受任何 [`sea_query::SimpleExpr`](https://docs.rs/sea-query/0.8/sea_query/expr/enum.SimpleExpr.html) 和一个别名。利用 [`sea_query::Expr`](https://docs.rs/sea-query/0.8/sea_query/expr/struct.Expr.html) 辅助函数构建 `SimpleExpr`。
...
你可以使用从 `FromQueryResult` 特征派生的自定义 `struct` 来处理复杂查询的结果。当处理无法直接转换为模型的自定义列或多个连接时，这尤其有用。它可以用于接收任何查询的结果，甚至是原始 SQL。
