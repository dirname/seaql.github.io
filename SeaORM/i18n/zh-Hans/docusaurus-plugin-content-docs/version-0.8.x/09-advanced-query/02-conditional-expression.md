...
使用 `Condition::all` 方法构建 AND 条件表达式，并通过 `add` 方法附加任何由 [`sea_query::SimpleExpr`](https://docs.rs/sea-query/0.8/sea_query/expr/enum.SimpleExpr.html) 表示的条件。
...
使用 `Condition::any` 方法构建 OR 条件表达式，并通过 `add` 方法附加任何由 [`sea_query::SimpleExpr`](https://docs.rs/sea-query/0.8/sea_query/expr/enum.SimpleExpr.html) 表示的条件。
