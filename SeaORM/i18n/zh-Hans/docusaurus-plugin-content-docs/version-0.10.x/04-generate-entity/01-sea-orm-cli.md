...
在您的环境中设置 `DATABASE_URL`，或者在项目根目录下创建一个 `.env` 文件。指定数据库连接。
...
命令行选项：
- `-u` / `--database-url`：数据库 URL（默认：环境变量中指定的 DATABASE_URL）
- `-s` / `--database-schema`：数据库模式（默认：环境变量中指定的 DATABASE_SCHEMA）
    - 对于 MySQL 和 SQLite，此参数将被忽略
    - 对于 PostgreSQL，此参数是可选的，默认值为 'public'
- `-o` / `--output-dir`：实体文件输出目录（默认：当前目录）
- `-v` / `--verbose`：打印调试消息
- `-l` / `--lib`：将索引文件生成为 `lib.rs` 而不是 `mod.rs`
- `--include-hidden-tables`：从隐藏表生成实体文件（默认情况下，以下划线开头的表名被视为隐藏且被忽略）
...
- `--date-time-crate`：用于生成实体的日期时间包（`chrono`，`time`）（默认：`chrono`）
