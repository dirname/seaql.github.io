...
通过实现 [`EntityTrait`](https://docs.rs/sea-orm/*/sea_orm/entity/trait.EntityTrait.html)，你可以在给定的表上执行 CRUD 操作。
...
`ActiveModel` 包含其对应的 `Model` 的所有属性，但所有属性都被包裹在了一个 [`ActiveValue`](https://docs.rs/sea-orm/*/sea_orm/entity/enum.ActiveValue.html) 中。
...
在 `ActiveModel` 上不同触发动作的处理器。例如，你可以执行自定义验证逻辑，阻止一个模型保存到数据库中。即使在一个事务内部，你也可以在操作完成后中断它。
...
定义特质边界以帮助你一起查询相关实体，特别是在多对多关系中特别有帮助。
