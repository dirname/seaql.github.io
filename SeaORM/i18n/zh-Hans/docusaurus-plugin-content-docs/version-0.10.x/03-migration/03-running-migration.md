...
运行在 `migration/main.rs` 中定义的迁移动态库 CLI。
...
你可以在应用程序启动时使用 `Migrator` 执行迁移，它实现了 `MigratorTrait` [trait](https://docs.rs/sea-orm-migration/*/sea_orm_migration/migrator/trait.MigratorTrait.html)。
...
默认情况下，迁移将在 `public` 架构上运行，现在你可以在通过 CLI 或编程方式运行迁移时覆盖它。
...
let connect_options = ConnectOptions::new("postgres://root:root@localhost/database".into())
    .set_schema_search_path("my_schema".into()) // 覆盖默认架构
    .to_owned();
