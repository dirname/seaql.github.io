...
建议您按照以下方式构建 cargo 工作区，以便在 app crate 和迁移 crate 之间共享 SeaORM 实体。请查阅 [集成示例](https://github.com/SeaQL/sea-orm/tree/master/examples) 以获取演示。
...
您可以创建一个没有实体文件的实体 crate。然后，编写迁移并运行它以在数据库中创建表。最后，使用 `sea-orm-cli` [生成 SeaORM 实体](04-generate-entity/01-sea-orm-cli.md)，并将实体文件输出到 `entity/src/entities` 文件夹。
...
创建一个包含 app、entity 和 migration crate 的工作区，并将 entity crate 导入 app crate。

如果我们希望将迁移工具作为应用程序的一部分打包，您还可能需要导入迁移 crate。
