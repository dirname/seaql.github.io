...
我们使用 `MockDatabase::new(DatabaseBackend::Postgres)` 创建一个针对PostgreSQL的模拟数据库。然后，使用 `append_query_results` 方法准备查询结果。请注意，我们将向其传递一个向量的向量，表示多个查询结果，每个结果中有多个模型。最后，我们将其转换为连接，并像使用正常的实时连接一样用它来进行CRUD操作。
...
这与模拟查询结果非常相似，不同之处在于我们在这里使用了 `append_exec_results` 方法，并且我们在单元测试中执行插入、更新和删除操作。`append_exec_results` 方法接受一个 `MockExecResult` 的向量，其中每一个都代表相应操作的执行结果。
