...
默认情况下，SeaORM 将选择在 `Column` 枚举中定义的所有列。如果只想选择特定的列，你可以覆盖这些默认设置。
...
使用 `select_only` 和 `column` 方法结合来只选择你想要的属性。
...
你可以使用从 `FromQueryResult` 特征派生的自定义 `struct` 来处理复杂查询的结果。当你处理无法直接转换为模型的自定义列或多个联接时，这特别有用。它可以用来接收任何查询的结果，甚至是原始 SQL。
