...
您可以使用 `join` 方法来构建复杂的连接查询。它接受在实体文件中定义的任何 `RelationDef`，并且您也可以使用 `belongs_to` 方法来定义关系。通过 `JoinType` 如内连接、左连接和右连接来指定连接类型。
...
> 您可以使用源自 `FromQueryResult` 特性的自定义 `struct` 来处理这种复杂查询的结果。详细信息请参见[这里](08-advanced-query/01-custom-select.md#handling-custom-selects)。