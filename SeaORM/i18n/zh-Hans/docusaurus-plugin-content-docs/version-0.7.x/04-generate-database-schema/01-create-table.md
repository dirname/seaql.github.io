...
为了在数据库中创建表而不必手动编写[`TableCreateStatement`](https://docs.rs/sea-query/*/sea_query/table/struct.TableCreateStatement.html)，你可以使用`Entity`派生它，通过调用[`Schema::create_table_from_entity`](https://docs.rs/sea-orm/0.7/sea_orm/schema/struct.Schema.html#method.create_table_from_entity)。这个方法将帮助你创建一个数据库表，包括在`Entity`中定义的所有列和外键约束。
...
请注意，自版本`0.7.0`起，[`Schema::create_table_from_entity`](https://docs.rs/sea-orm/0.7/sea_orm/schema/struct.Schema.html#method.create_table_from_entity)不再创建索引。如果你需要在数据库中创建索引，请查阅[这里](04-generate-database-schema/03-create-index.md)了解详情。
...
为了进一步说明这一点，我们将在下面展示作为字符串的SQL语句。
