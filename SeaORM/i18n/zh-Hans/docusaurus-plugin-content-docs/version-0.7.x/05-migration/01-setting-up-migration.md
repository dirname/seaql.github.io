...
使用以 SeaQuery 或原始 SQL 编写的迁移来对数据库模式进行版本控制。

## 迁移表

在你的数据库中会创建一个名为 `seaql_migrations` 的表，用于跟踪已应用的迁移。当你运行迁移时，它会自动创建。
...
初始化迁移目录...
正在创建文件 `./other/migration/dir/src/lib.rs`
正在创建文件 `./other/migration/dir/src/m20220101_000001_create_table.rs`
正在创建文件 `./other/migration/dir/src/main.rs`
正在创建文件 `./other/migration/dir/Cargo.toml`
正在创建文件 `./other/migration/dir/README.md`
完成！
```
...
├── README.md
└── src
    ├── lib.rs                              # 迁移器，用于程序化运行迁移
    ├── m20220101_000001_create_table.rs    # 一个示例迁移文件
    └── main.rs                             # 迁移器 CLI，用于在控制台运行迁移
```

## 工作区结构

建议你按照以下方式重构你的 cargo 工作区，以便在核心包和迁移包之间共享 SeaORM 实体。同时，确保它们都通过重导出依赖于相同版本的 SeaORM。
...
在你的根工作区中创建一个实体包。它应该包含所有 SeaORM 实体，并通过重导出在整个工作区共享 SeaORM 依赖。
...
生成实体文件后，可以通过在 `entity/src/lib.rs` 中添加以下行来重导出生成的实体：
...
对于现有的 SeaORM 用户，在定义迁移时可能需要 SeaORM 实体。例如，可以在迁移中重用在实体中定义的列名。
...
为实体编写迁移。有关更多详细信息，请参阅 [编写迁移](./02-writing-migration.md)。
