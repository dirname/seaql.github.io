...
迁移可以在终端中手动运行。`DATABASE_URL` 必须在您的环境中设置，按照 [此处](03-generate-entity/01-sea-orm-cli.md#configure-environment) 的说明进行配置。
...
您可以通过 `Migrator` 在应用程序启动时执行迁移，鉴于它实现了 [`MigratorTrait`](https://docs.rs/sea-schema/0.5/sea_schema/migration/migrator/trait.MigratorTrait.html)。
