...
或者你可以使用以下模板创建一个迁移文件。根据命名规范 `mYYYYMMDD_HHMMSS_migration_name.rs` 来命名文件。
...
此外，你必须将新的迁移包含在 [`MigratorTrait::migrations`](https://docs.rs/sea-orm-migration/0.9/sea_orm_migration/migrator/trait.MigratorTrait.html#tymethod.migrations) 方法中。请注意，迁移应按时间顺序排序。
...
查看 [`SchemaManager`](https://docs.rs/sea-orm-migration/0.9/sea_orm_migration/manager/struct.SchemaManager.html) 以获取 API 参考。

### SeaQuery

点击 [这里](https://github.com/SeaQL/sea-query#table-create) 快速浏览 SeaQuery 的 DDL 语句。

你需要使用 [`sea_query::Iden`](https://github.com/SeaQL/sea-query#iden) 来定义将在你的迁移中使用的标识符。
...
从总体上看，我们推荐采用先有模式的方法：首先编写迁移，然后从活动数据库生成实体。

有时，你可能希望使用 [`create_*_from_entity`](09-generate-sea-query-statement/01-create-table.md) 方法来通过几个手写的实体文件初始化你的数据库。

如果你打算永远不改变实体模式，这样做完全没问题。或者，你可以克隆原始实体，并在迁移文件中依赖它。