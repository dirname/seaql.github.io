...
请注意 `db_type` 和额外的 `enum_name` 属性。

### PostgreSQL

PostgreSQL 中的枚举由 [`TypeCreateStatement`](https://docs.rs/sea-query/0.9/sea_query/extension/postgres/struct.TypeCreateStatement.html) 定义，可以从 `Entity` 通过 [`Schema::create_enum_from_entity`](https://docs.rs/sea-orm/0.9/sea_orm/schema/struct.Schema.html#method.create_enum_from_entity) 方法创建。

您也可以从 `ActiveEnum` 通过 [`Schema::create_enum_from_active_enum`](https://docs.rs/sea-orm/0.9/sea_orm/schema/struct.Schema.html#method.create_enum_from_active_enum) 方法创建它。
...
在 MySQL 中，枚举是在创建表时定义的，因此只需调用一次 [`Schema::create_table_from_entity`](https://docs.rs/sea-orm/0.9/sea_orm/schema/struct.Schema.html#method.create_table_from_entity)。
