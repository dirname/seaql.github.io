...
为了在数据库中创建表而不必手动编写[`TableCreateStatement`](https://docs.rs/sea-query/0.9/sea_query/table/struct.TableCreateStatement.html)，你可以使用`Entity`派生它，通过调用[`Schema::create_table_from_entity`](https://docs.rs/sea-orm/0.9/sea_orm/schema/struct.Schema.html#method.create_table_from_entity)。这个方法将帮助你创建一个数据库表，包括`Entity`中定义的所有列和外键约束。

下面我们使用[`CakeFillingPrice`](https://github.com/SeaQL/sea-orm/blob/master/src/tests_cfg/cake_filling_price.rs)实体来演示其生成的SQL语句。你可以使用相同的语句构建[`TableCreateStatement`](https://docs.rs/sea-query/0.9/sea_query/table/struct.TableCreateStatement.html)。

请注意，自版本`0.7.0`起，[`Schema::create_table_from_entity`](https://docs.rs/sea-orm/0.9/sea_orm/schema/struct.Schema.html#method.create_table_from_entity)不再创建索引。如果你需要在数据库中创建索引，请查阅[这里](09-generate-sea-query-statement/03-create-index.md)了解详情。
