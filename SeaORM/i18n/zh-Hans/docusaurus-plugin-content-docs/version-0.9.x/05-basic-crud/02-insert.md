...
如果您想将用户输入保存到数据库中，可以轻松地将JSON值转换为`ActiveModel`。需要注意的是，您可能想要[跳过反序列化](https://serde.rs/attr-skip-serializing.html)JSON的主要键属性，如下所示，您可以进行配置。
...
您可以使用[`is_changed`](https://docs.rs/sea-orm/0.9/sea_orm/entity/prelude/trait.ActiveModelTrait.html#method.is_changed)方法检查`ActiveModel`中的任何字段是否被`Set`。
...
插入一个活动模型并返回一个新鲜的`Model`。它的值是从数据库中检索的，因此任何自动生成的字段都将被填充。
