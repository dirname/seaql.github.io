...
Diesel 提供了一个全编译时的 API，其中类型可以完全静态检查。您也可以使用 Diesel 进行动态查询，但会失去一些编译时类型检查的优势。

SeaORM 是动态的，其中事物在运行时建立。它提供了更多灵活性。虽然您会失去一些编译时的检查，但 SeaORM 通过测试帮助您证明正确性。

这两个库都大量使用了特质和泛型，但 SeaORM 生成的类型较少：Diesel 中的每一列都是一个结构体（每个结构体实现了几个特质），而 SeaORM 中的每一列都是枚举的一个变体（实体的所有列共同形成一个枚举，该枚举实现了一些特质）。
...
Diesel 是 Rust 生态系统中一个成熟稳定的库。SeaORM 非常新。两者都无法取代对方。我们希望 Rust 社区能够一起变得更加强大。