...
我们使用 `MockDatabase::new(DatabaseBackend::Postgres)` 创建一个针对 PostgreSQL 的模拟数据库。然后，使用 `append_query_results` 方法准备查询结果。请注意，我们向其传递了一个向量的向量，代表多个查询结果，每个结果包含一个以上的模型。最后，我们将它转换为连接，并像使用正常的实时连接一样进行CRUD操作。

`MockDatabase` 的一个特别之处在于你可以检查它的事务日志。在模拟数据库上运行的任何SQL查询都会被记录下来；你可以验证每一项日志，以确保你的应用程序逻辑的正确性。
...
这与模拟查询结果非常相似，不同之处在于我们在这里使用了 `append_exec_results` 方法，并且在此单元测试中执行插入、更新和删除操作。`append_exec_results` 方法接受一个 `MockExecResult` 的向量，其中每一个代表对应操作的执行结果。
