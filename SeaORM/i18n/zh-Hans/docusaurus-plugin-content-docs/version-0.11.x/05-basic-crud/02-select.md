...
通过其主键查找模型，它可以是单个键或复合键。我们从调用 `Entity` 上的 [`find_by_id`](https://docs.rs/sea-orm/*/sea_orm/entity/trait.EntityTrait.html#method.find_by_id) 开始，它帮助你自动构建查询和条件。然后，使用 `one` 方法从数据库中获取单个模型。
...
除了通过主键检索模型外，你还可以根据特定条件按一定顺序检索一个或多个模型。`find` 方法让你可以访问 SeaORM 中的查询构建器。它支持构造所有常见的选择表达式，如 `where` 和 `order by`。它们可以分别使用 `filter` 和 `order_by_*` 方法构建。

> 阅读更多关于 [条件表达式](08-advanced-query/02-conditional-expression.md) 的内容。
...
> 在 [关系](06-relation/01-one-to-one.md) 章节中阅读更多。

### 懒加载

使用 `find_related` 方法。

当你请求它们时，相关模型会按需加载，如果你想要基于某些应用程序逻辑加载相关模型，这是首选的。请注意，与急加载相比，懒加载会增加数据库往返次数。
...
所有相关模型一次性加载。这相比懒加载提供了最少的数据库往返次数。
...
使用 `find_with_related` 方法，相关模型将按照父模型分组。为了实现这一点，查询按照蛋糕的主键进行排序。此方法处理 1-N 和 M-N 情况，在涉及关联表的情况下将执行两次联接。
...
自 0.11 版本开始，我们引入了一个 [LoaderTrait](https://docs.rs/sea-orm/*/sea_orm/query/trait.LoaderTrait.html) 来批量加载相关实体。

与急加载相比，它节省了带宽（考虑一对多的情况，一侧的行可能会重复），但以额外一次（或两次，在多对多的情况下）数据库往返为代价。
...
使用 `load_many_to_many` 方法。你必须提供关联实体。

```rust
let 蛋糕: Vec<cake::Model> = Cake::find().all(db).await?;
let 填充物: Vec<Vec<filling::Model>> = 蛋糕.load_many_to_many(Filling, CakeFilling, db).await?;
```
...
while let Some(蛋糕) = 蛋糕页面.fetch_and_next().await? {
    // 在 蛋糕: Vec<cake::Model> 上做一些事情
}
```

## 游标分页

如果你想根据列（如主键）分页行，请使用游标分页。
...
// 过滤分页结果：`蛋糕`.`id` > 1 且 `蛋糕`.`id` < 100
游标.after(1).before(100);

// 获取前 10 行（按 `蛋糕`.`id` 升序排列）
for 蛋糕 in 游标.first(10).all(db).await? {
    // 在 蛋糕: cake::Model 上做一些事情
}

// 获取最后 10 行（按 `蛋糕`.`id` 降序排列，但行是以升序返回的）
