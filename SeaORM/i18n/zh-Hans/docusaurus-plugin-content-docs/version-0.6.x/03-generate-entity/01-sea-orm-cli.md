...
- `--compact-format`：生成实体文件的 [紧凑格式](03-generate-entity/02-entity-structure.md) （默认：true）
- `--expanded-format`：生成实体文件的 [扩展格式](03-generate-entity/03-expanded-entity-structure.md)
- `--with-serde`：自动为实体推导 serde Serialize / Deserialize 特征（`none`，`serialize`，`deserialize`，`both`）（默认：`none`）
