...
| Rust 类型 | 数据库类型 <br/> ([`ColumnType`](https://docs.rs/sea-orm/0.6/sea_orm/entity/enum.ColumnType.html)) |
| --------- | ------------- |
| String | Char |
| String | String |
| u8, i8 | TinyInteger |
...
如果你想忽略特定的模型属性，使其不映射到任何数据库列，你可以使用 `ignore` 注解。
...
这通常发生在连接表中，其中使用两列元组作为主键。只需注释多个列来定义复合主键。默认情况下，对于复合键，`auto_increment` 是 `false`。
