...
为了确保你的应用程序逻辑的正确性，你还可以验证模拟数据库中的事务日志。

请查看我们如何使用模拟连接编写单元测试[这里](https://github.com/SeaQL/sea-orm/blob/master/src/executor/paginator.rs#L250)。

## 模拟查询结果

我们使用`MockDatabase::new(DatabaseBackend::Postgres)`为PostgreSQL创建一个模拟数据库。然后，使用`append_query_results`方法准备查询结果。请注意，我们向其传递了一个包含多个向量的向量，代表了多个查询结果，每个结果中有不止一个模型。最后，我们将它转换为连接，并像使用常规实时连接一样用它来进行CRUD操作。

`MockDatabase`的一个特别之处在于你可以检查它的事务日志。在模拟数据库上运行的任何SQL查询都将被记录；你可以验证每一条日志以确保你的应用程序逻辑的正确性。
...
这与模拟查询结果非常相似，不同之处在于我们在这里使用`append_exec_results`方法，并且在单元测试中执行插入、更新和删除操作。`append_exec_results`方法接收一个`MockExecResult`的向量，其中每一个都代表相应操作的执行结果。
