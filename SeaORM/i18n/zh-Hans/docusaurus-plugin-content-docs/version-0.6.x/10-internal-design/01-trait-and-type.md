...
一个枚举，实现了`PrimaryKeyTrait`，代表主键。每个主键变体必须有对应的列变体。
...
一个枚举，实现了`ActiveEnum`，代表作为Rust枚举变体存储在数据库中的值。
...
一个泛型特质，`Related`，定义了连接路径来帮助你一起查询相关实体，特别是在多对多关系中尤其有用。

## 链接 Linked

一个特质，`Linked`，定义了复杂的连接路径，包括链式关联、自引用关联以及两个实体之间的多个关联。