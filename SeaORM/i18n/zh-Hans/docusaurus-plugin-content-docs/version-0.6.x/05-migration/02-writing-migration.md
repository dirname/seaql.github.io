...
你可以通过复制现有的迁移文件或下面的模板来创建迁移。请记住按照命名规范 `mYYYYMMDD_HHMMSS_migration_name.rs` 命名文件，并相应地更新 [`MigrationName::name`](https://docs.rs/sea-schema/0.5/sea_schema/migration/trait.MigrationName.html#tymethod.name) 方法。
...
同时，你必须在 [`MigratorTrait::migrations`](https://docs.rs/sea-schema/0.5/sea_schema/migration/migrator/trait.MigratorTrait.html#tymethod.migrations) 方法中包含新的迁移。注意迁移应该按时间顺序排序。
...
[`SchemaManager`](https://docs.rs/sea-schema/0.5/sea_schema/migration/manager/struct.SchemaManager.html) 帮助你在 SeaQuery 或原始 SQL 中定义迁移。

### SeaQuery

点击 [这里](https://github.com/SeaQL/sea-query#usage) 快速浏览 SeaQuery。
