...
`Related` 特性表示我们在实体关系图中绘制的箭头（1-1、1-N、M-N）。一个 [`Linked`](https://docs.rs/sea-orm/0.6/sea_orm/entity/trait.Linked.html) 由一系列关系组成，在以下情况下很有用：
...
使用 [`find_linked`](https://docs.rs/sea-orm/0.6/sea_orm/entity/prelude/trait.ModelTrait.html#method.find_linked) 方法查找可以填充到蛋糕中的填充物。
...
使用 [`find_also_linked`](https://docs.rs/sea-orm/0.6/sea_orm/entity/prelude/struct.Select.html#method.find_also_linked) 方法在一个单一的 SELECT 中找到所有蛋糕和填充物的配对。
